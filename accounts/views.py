...
class RegisterView(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        # Extract user_type and grade from request
        user_type = request.data.get('user_type')
        grade = request.data.get('grade')
        
        # Validate that students select a grade
        if user_type == 'STUDENT' and grade not in ['4', '9', '11']:
            return Response(
                {"error": "Students must select a grade (4, 9, or 11)"},
                status=status.HTTP_400_BAD_REQUEST
            )
        
        # If user is a teacher, set grade to None
        ...
        if serializer.is_valid():
            user = serializer.save(user_type=user_type, grade=grade)
